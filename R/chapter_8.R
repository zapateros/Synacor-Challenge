...
inputs <-c("doorway", "north", "north", "bridge", "continue", "down", "east", "take empty lantern", "west",
           ...
           "take teleporter", "use teleporter", "take business card", "take strange book")
input_chars <- unlist(strsplit(paste0(inputs,"0"),""))
input_chars <- gsub("0","\n", input_chars)
run_vm()

#calculation of the correct register 8. This takes about 15 minutes so you can also start iterating
#from reg8 = 25731 or so to speed it up
reg8 <- 25731
while(TRUE){
  reg8 <- reg8 + 1
  b    <- reg8
...
inputs <-c("west", "north", "north", "north", "north", "north", "north", "north", "east", "take journal",
           "west", "north", "north", "take orb","north", "east", "east", "north", "west", "south", "east",
           "east", "west", "north", "north", "east", "vault", "take mirror", "use mirror")
input_chars <- unlist(strsplit(paste0(inputs,"0"),""))
input_chars <- gsub("0","\n", input_chars)
run_vm()
